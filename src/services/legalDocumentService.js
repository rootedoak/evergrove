...
export async function getPublishedLegalDocument(documentType) {
    const { data, error } = await supabase
        .from("legal_document_versions")
        .select(`
            id,
...
export async function getRequiredLegalAcceptances() {
    ...
    const { data: requiredDocuments, error: documentError } =
        await supabase
            .from("legal_document_versions")
            .select(`
                id,
    ...
    const { data: acceptances, error: acceptanceError } =
        await supabase
    ...
    const acceptedDocumentIds = new Set(
        (acceptances ?? []).map(
            acceptance => acceptance.legal_document_version_id
        )
    )
...
export async function acceptLegalDocuments({
    ...
    const rows = documents.map(document => ({
        ...
        adult_eligibility_confirmed: adultEligibilityConfirmed,
        household_authority_confirmed: householdAuthorityConfirmed,
    ...
    const { error } = await supabase
        .from("user_legal_acceptances")
        .upsert(rows, {
            onConflict: "user_id,legal_document_version_id",
...
export async function recordLegalAttestations(attestations) {
    const {
        data: { user },
        error: userError
    } = await supabase.auth.getUser()
